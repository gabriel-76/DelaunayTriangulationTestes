...
int firstlownode;
int currentlownode;
extern void buildKdtreeAndSortNode(int bend, int tend, int level, int nodeposition);
...
void getLclosestPoint(double x, double y, double z, int &icp) {
    double d0 = 1.e+30, d;
    for (int i = 0; i < numnodes3; i++) {
        if ((d = Ldistance(x, y, z, coord_nodes3[i][0], coord_nodes3[i][1], coord_nodes3[i][2])) < d0) {
            d0 = d;
            icp = i;
        }
    }
...
void makeInsOrder(void) {
    ...
    //printf("The time differencebso is: %lf seconds\n", double(second - first)); //difftime(second,first));
    int a = numnodes3;
    firstlownode = 1;
...
int getDirectFromArray(int bend, int tend) {
    ...
    if (sortway == 0) {
        for (int i = 0; i < 3; i++)
            if ((d = coord_nodes3[nodesort[i][tend]][i] - coord_nodes3[nodesort[i][bend]][i]) > d0) {
                dir = i;
                d0 = d;
            }
        return dir;
...
(const int &a, const int &b) {
    ...
    //if(coord_nodes3[a][(direction+1)%3]<coord_nodes3[b][(direction+1)%3]) return -1;
    //if(coord_nodes3[a][(direction+1)%3]>coord_nodes3[b][(direction+1)%3]) return 1;

    //if(coord_nodes3[a][(direction+2)%3]<coord_nodes3[b][(direction+2)%3]) return -1;
    //if(coord_nodes3[a][(direction+2)%3]>coord_nodes3[b][(direction+2)%3]) return 1;
...
int qsortNodeInDirect(const void *ai, const void *bi) {
    ...
    if (coord_nodes3[a][direction] < coord_nodes3[b][direction])
        return -1;
