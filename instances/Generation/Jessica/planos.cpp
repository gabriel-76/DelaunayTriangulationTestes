...
void handleResize(int w, int h) {
    ...
    gluPerspective(5.0,                  //The camera angle
                   5, //The width-to-height ratio
...
void drawScene() {
	...
	glRotatef(15.0f, 15.0f, 5.0f, 1.0f);
...
int main(int argc, char** argv) {
	...
	for(int i=valueX; i<valueY+valueX; i++){
		...
		for(int k=valueX; k<i; k++){
			if(coordinates[k].x==x && coordinates[k].z==z){
				previouslyInserted = true;
				i--;
				break;
			}
	...
	for(int i=valueY+valueX; i<nmbPts; i++){
		x = 1;
		while(x>0.05){
			x = rand()%100000000/(100000000*1.0);
		}
		y = rand()%1000000/(1000000*1.0);
		z = rand()%1000000/(1000000*1.0);
		bool previouslyInserted = false;
		for(int k=valueY+valueX; k<i; k++){
			if(coordinates[k].z==z && coordinates[k].y==y){
				previouslyInserted = true;
				i--;
				break;
			}
    ...
    //Create the window
    glutCreateWindow("Entrada em torno dos planos");
